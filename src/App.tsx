...
function App() {
  ...
  // Verificar si hay juego guardado al iniciar
  useEffect(() => {
    const savedGame = localStorage.getItem('catanCosmico');
    if (savedGame) {
      try {
        const parsed = JSON.parse(savedGame);
        // Verificar si el juego guardado es válido
    ...
    return (
      <div className="min-h-screen w-full flex items-center justify-center p-4">
        <div className="max-w-md w-full bg-gradient-to-br from-purple-900/50 to-blue-900/50 p-8 rounded-2xl border-2 border-purple-500/30">
          ...
          <button
            onClick={startGame}
            className="w-full px-6 py-4 bg-gradient-to-r from-purple-600 to-blue-600 rounded-lg font-bold text-lg hover:scale-105 transition-all flex items-center justify-center gap-2 mb-4"
          >
          ...
          {hasSavedGame && (
            <button
              ...
              className="w-full px-6 py-4 bg-gradient-to-r from-green-600 to-teal-600 rounded-lg font-bold text-lg hover:scale-105 transition-all flex items-center justify-center gap-2"
            >
